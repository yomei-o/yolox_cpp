...
def fuse(bc):
    ...
    return (w, b, conv.kernel_size[0], conv.stride[0], 1)
def emit(bc): convs.append(fuse(bc))
def emit_plain(c): convs.append((c.weight, c.bias, c.kernel_size[0], c.stride[0], 0))
def emit_csp(csp):
